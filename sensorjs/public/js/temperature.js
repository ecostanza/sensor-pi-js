document.addEventListener("DOMContentLoaded", function() { 
    ...
    let svgWidth = window.innerWidth/2 //+200; //> 700 ? 700:window.innerWidth ;
    ...
    const svgMarginBottom = 75//window.innerHeight > 530 ? 300:200;
    ...
    const appendDIV = function (measurement){
        svgContainer = d3.select("div#container")
            .append("div")
            .attr('class','nodeContainer col-6')
    ...
    function drawGraphs (response,sensor_id,series){
        ...
        xScale = d3.scaleTime(
            // [min, max],
            d3.extent(data[series.id], d => new Date(d.time.getTime())),
            [svgMarginLeft, svgWidth-svgMarginLeft]
        );

        if( series.measurement == 'humidity'){
        ...
        let xAxis = d3.axisTop()
            ...
            .tickFormat(d3.timeFormat('%e/%m %H:%M'))
            .scale(xScale);
        ...
        updateGraph = function (dataF, firstCall,id){
            ...
            var filteredData = dataF.filter(line.defined());
                    ...
                    if( d3.select('#dialogueBox').attr('isCreate') == 'true'){
                        ...
                        async function createNewAnnotation(eventSeries,eventSensor_id,eventMeasurement)
                        {
                            ...
                            try{
                                ...
                                addAnnotationBar(event,eventSanitized.id, eventSeries);
...
*/
    ...
    function addAnnotationBar(event,id,seriesID){
        ...
        anntContainer = d3.select('#'+seriesID+'Chart .annotations')
